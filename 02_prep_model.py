...
use_stations = list(data[OBS][STAT].keys())
# add entry for model data
data[MODEL] = {STAT:{}}
data[MODEL]['param_names'] = model_params
...
for i,_ in enumerate(mod_stations):
    ...
    try:
        ...
        if stat_key in use_stations:
            print('use ' + stat_key)
            data[MODEL][STAT][stat_key] = {PAR:{}}
            # add to data dictionary
            for j,mp in enumerate(model_params):
                data[MODEL][STAT][stat_key][PAR][mp] = values[:,j]
        else:
...
data['station_names'] = list(data[MODEL][STAT].keys())
